...
if ($PSVersionTable.Platform -eq "Unix" -or $IsWindows -eq $false -or (Test-Path "/bin/bash")) {
    $HOOK_SOURCE = Join-Path $SCRIPT_DIR "scripts\pre-commit"
    $HOOK_TARGET = Join-Path $HOOKS_DIR "pre-commit"
} else {
    # Windows PowerShell
    $HOOK_SOURCE = Join-Path $SCRIPT_DIR "scripts\pre-commit.ps1"
    $HOOK_TARGET = Join-Path $HOOKS_DIR "pre-commit"
}
...
Write-Host ""
Write-Host "SUCCESS: pre-commit hook installed"
Write-Host ""
Write-Host "The hook will validate project.yaml before each commit."
Write-Host "To uninstall: rm .git/hooks/pre-commit"
exit 0